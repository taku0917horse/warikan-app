...
- **支出記録** — 誰がいくら払ったかを記録できます。均等割りのほか、「傾斜配分（個別に設定）」で各メンバーの負担額を個別指定することも可能です
...
金額を人数で均等に分けたくない場合は「傾斜配分（個別に設定）」にチェックを入れると、各メンバーの負担額を個別に入力できます。入力した負担額の合計が金額と一致していないと追加できません。

### 5. 精算を確認する

「精算」タブで合計金額・各自の収支・精算方法が自動で表示されます。
